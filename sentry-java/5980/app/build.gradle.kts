...
// Narrow the instrumentation to single features to see which one costs what, e.g.
//   ./gradlew :app:installDebug -PsentryFeatures=DATABASE
//   ./gradlew :app:installDebug -PsentryFeatures=FILE_IO
val instrumentationFeatures =
  (project.findProperty("sentryFeatures") as String? ?: "DATABASE,FILE_IO,OKHTTP,COMPOSE")
    .split(",")
    .filter { it.isNotBlank() }
    .map { InstrumentationFeature.valueOf(it.trim()) }
    .toSet()

android {
  namespace = "io.repro.issue5980"
  compileSdk = 36

  defaultConfig {
    applicationId = "io.repro.issue5980"
    minSdk = 24
    targetSdk = 36
    versionCode = 1
    versionName = "1.0"
    ...
    buildConfigField(
      "String",
      "SENTRY_FEATURES",
      "\"${if (instrumentationEnabled) instrumentationFeatures.joinToString("+") else "none"}\"",
    )
...
sentry {
  ...
  tracingInstrumentation {
    ...
    features.set(instrumentationFeatures)
  }
}
